...
public interface AnimalDAO extends JpaRepository<Animal, Integer> {
	...
	@Query("Select a "//
			+ "From Animal a Join a.proprietaire p"//
			+ " Where p.lastName Like CONCAT('%',:nomProprietaire,'%')")
	List<Animal> findAllNomProprietaire(@Param("nomProprietaire") String nomProprietaire);

	@Query("Select a "//
			+ "From Animal a "//
			+ "Where a.class = :typeAnimal")
	List<Animal> findAllTypeAnimal(@Param("typeAnimal") String typeAnimal);

	@Query("Select a "//
			+ "From Animal a "//
			+ "Where SUBSTRING(a.dateDeNaissance, 1, 4)  = SUBSTRING(:dateDeNaissanceAnimal, 1, 4)")
